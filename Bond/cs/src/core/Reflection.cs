...
namespace Bond
{
    ...
    public static class Reflection
    {
        ...
        {
            Type Alias;
            if ((Alias = CustomTypeRegistry.GetTypeMapping(type)) != null)
            {
                return GetSchemaName(Alias);
            }
        ...
        static Type ResolveTypeArgumentTagsWithCustomRegistry(Type memberType, Type schemaType)
        {
            schemaType = CustomTypeRegistry.GetTypeMapping(schemaType) ?? schemaType;
            ...
            if (schemaType.IsGenericType())
            {
                ...
                return schemaGenericType.MakeGenericType(Enumerable.Zip(
                    memberTypeArguments,
                    schemaType.GetTypeInfo().GenericTypeArguments,
                    ResolveTypeArgumentTagsWithCustomRegistry).ToArray());
            }
